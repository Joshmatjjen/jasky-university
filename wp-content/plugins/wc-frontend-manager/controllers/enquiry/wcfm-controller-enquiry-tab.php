	...
	public function processing() {
	  ...
	  if(isset($wcfm_enquiry_tab_form_data['enquiry']) && !empty($wcfm_enquiry_tab_form_data['enquiry'])) {
		  ...
		  //$headers[] = 'Cc: ' . $customer_email;
			...
			// Direct message
			$wcfm_messages = sprintf( __( 'You have a recent enquiry for <b>%s</b>', 'wc-frontend-manager' ), $enquiry_for );
			...
			if( wcfm_is_marketplace() ) {
				if( $vendor_id ) {
					...
					if( $is_allow_enquiry && apply_filters( 'wcfm_is_allow_enquiry_vendor_notification', true ) ) {
						...
						// Direct message
						$wcfm_messages = sprintf( __( 'You have received an Enquiry for <b>%s</b>', 'wc-frontend-manager' ), $enquiry_for );
