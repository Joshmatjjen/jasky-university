    ...
    if( $status_filter ) {
		...
		if(!empty($wcfm_withdrawal_requests_array)) {
			...
			foreach( $wcfm_withdrawal_requests_array as $wcfm_withdrawal_request_single ) {
				...
				// Transc.ID
				$wcfm_withdrawal_requests_json_arr[$index][] = '<a href="' . wcfm_transaction_details_url( $wcfm_withdrawal_request_single->ID ) . '" class="wcfm_dashboard_item_title"># ' . sprintf( '%06u', $wcfm_withdrawal_request_single->ID ) . '</a>';
