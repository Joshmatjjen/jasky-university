	...
	public function wcfm_get_vendor_store_by_vendor( $vendor_id ) {
  	...
  	if( $marketplece == 'wcvendors' ) {
  		$shop_name = get_user_meta( $vendor_id, 'pv_shop_name', true );
			...
			if( $shop_name ) { $vendor_store = '<a class="wcfm_dashboard_item_title" target="_blank" href="' . apply_filters('wcfmmp_vendor_shop_permalink', $shop_link) . '">' . $shop_name . '</a>'; }
			else { $vendor_store = '<a class="wcfm_dashboard_item_title" target="_blank" href="' . apply_filters('wcfmmp_vendor_shop_permalink', $shop_link) . '">' . __('Shop', 'wc-frontend-manager') . '</a>'; }
		}
